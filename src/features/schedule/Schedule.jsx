...
const Schedule = () => {
    ...
    const { events, loading, error, lastUpdated, refreshData, getCacheAge, scheduleUrl } = useSchedule();
    ...
    // Show setup guide if no schedule URL configured and no events
    if (!loading && !error && events.length === 0 && !scheduleUrl) {
        return (
            <div className="space-y-6">
                <header>
                    <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Emploi du temps</h2>
                    <p className="text-slate-500 dark:text-slate-400">Synchronisez votre planning pour le visualiser ici</p>
                </header>
                <ScheduleSetupGuide />
            </div>
        );
