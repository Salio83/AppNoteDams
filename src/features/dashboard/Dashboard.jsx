...
// Recent grades card
const RecentGradesCard = ({ grades, ues }) => {
    const navigate = useNavigate();
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                {hasUEConfig && <RecentGradesCard grades={grades} ues={ues} />}
                <AlertsCard alerts={alerts} />
            </div>

            {/* Setup guides for new users */}
            {(!hasUEConfig || (!events || events.length === 0)) && (
                <div className="space-y-4">
                    {(!events || events.length === 0) && <ScheduleSetupGuide />}
